-- ============================================================
-- 0011_reservas_por_profesional.sql
--
-- Hasta acá las reservas online eran un interruptor de todo el centro
-- (centros.reservas_publicas): al prenderlo quedaba publicada la agenda
-- de TODOS los profesionales activos que tuvieran horarios cargados. En
-- un centro compartido eso no sirve: una puede querer recibir reservas
-- online y la otra no.
--
-- Ahora son dos niveles:
--   - centros.reservas_publicas: la llave general del centro (admin).
--   - perfiles.acepta_reservas_online: cada profesional decide si su
--     propia agenda se publica. Arranca en false, así nadie queda
--     expuesto sin haberlo elegido.
--
-- Las tres funciones públicas de reserva pasan a exigir las dos cosas.
-- Los cuerpos de abajo son los que estaban vivos en la base, con el
-- chequeo nuevo agregado (generados desde pg_get_functiondef para no
-- introducir diferencias al recopiarlos a mano).
--
-- Correr DESPUES de 0010. Es idempotente.
-- ============================================================

alter table perfiles
  add column if not exists acepta_reservas_online boolean not null default false;

-- Backfill: se preserva el comportamiento actual. Donde el centro ya
-- tenía las reservas online prendidas, sus profesionales siguen
-- publicados; donde estaban apagadas (CIK), cada una lo prende cuando
-- quiera desde Configuración.
do $$ begin
  if not exists (select 1 from perfiles where acepta_reservas_online) then
    update perfiles p set acepta_reservas_online = true
     where exists (select 1 from centros c
                    where c.id = p.centro_id and c.reservas_publicas);
  end if;
end $$;
